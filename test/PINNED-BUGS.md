...
| 3 | `onMessage` parses the frame with an unguarded `JSON.parse`, so a malformed frame throws straight out of the callback. The `if (!data)` guard below it — which logs `[ddp] JSON parse error` — can therefore never see bad JSON; only a falsy `e.data` (an empty string) reaches it, and the message it logs reads `e.message`, which a websocket message event does not carry. | `lib/drivers/ddp.ts`, `onMessage` | `lib/drivers/ddp.onMessage.spec.ts` pins the throw. The empty-payload branch is recorded here rather than tested — testing it would assert a log line that describes something that did not happen. | Fixing means wrapping the parse in a `try`, which is a shipped-source change. |
