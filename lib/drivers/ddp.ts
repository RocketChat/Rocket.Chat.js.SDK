...
export class Socket extends SDKEventEmitter {
  ...
  reopenNow = (): Promise<void> => {
    ...
    const reopening = this.awaitEvent('open', this.config.timeout, () => {
  ...
  private writePing = (): boolean => {
    if (!this.connection || this.connection.readyState !== socketOpen) return false
    try {
  ...
  probe = (deadlineMs = probeDeadline): Promise<boolean> => {
    return this.awaitEvent('pong', deadlineMs, this.writePing, 'close')
  }
  ...
  ping = () => {
    ...
    this.pingTimeout = setTimeout(async () => {
      ...
      const answered = await this.probe(this.config.ping)
      this.off('close', onClose)

      if (answered) return this.ping()
      // A close carries its own reopen decision; only a silent socket needs one here.
      if (closed) return
      this.reopen()
    }, this.config.ping)
  }
