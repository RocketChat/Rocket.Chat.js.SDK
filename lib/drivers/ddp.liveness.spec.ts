...
describe('Socket liveness', () => {
  ...
  describe('connected', () => {
    ...
    it('skips closing a stale-ping socket, leaking it open', async () => {
      // Known bug: `close` gates the actual close on `connected`, which a stale
  ...
  describe('probe', () => {
    ...
    it('ignores a pong that lands in the same millisecond as the probe', async () => {
      // Known bug: the probe snapshots `lastPing` and requires it to *advance*
      // strictly, so a pong answered within the same millisecond is discarded and
      // the probe times out on a demonstrably live socket. Under the frozen clock
