...
nor accept it. The socket being closed may be one that was never Transport open
at all. A close also runs while another connection may already have taken the
place of the one being closed, and while a wait on the old socket's `open` may
still be pending. Each of those cases leaves the driver holding a socket, a
