# ADR-0007: The websocket reference directive stays
...
The oxlint rule `triple-slash-reference` flags exactly this line, and it is the
only violation in the repo.
...
- Lint output for this repo is never empty. One `triple-slash-reference` warning
  is the clean state; a second one is not, and is worth reading, because no other
  file has a reason to reference a declaration this way.
