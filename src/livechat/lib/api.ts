...
export const livechat: any = {
  ...
  closeChat: (credentials: ILivechatRoomCredentialAPI) => (api.post(`livechat/room.close/${ credentials.rid }`, { token: credentials.token }, false)).then((r) => r),
  transferChat: (credentials: ILivechatRoomCredentialAPI) => (api.post(`livechat/room.transfer/${ credentials.rid }`, { token: credentials.token, department: credentials.department }, false)).then((r) => r),
  chatSurvey: (survey: ILivechatRoomSurveyAPI) => (api.post(`livechat/room.survey/${ survey.rid }`, { token: survey.token, data: survey.data }, false)).then((r) => r),
